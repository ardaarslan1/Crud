...
<div class="profile">
    <form method="<?=$method?>" action="">
        ...
        Password:<input type="password" name="password" maxlength="40" required value="<?php echo isset($_POST['password'])?>"><br><br>
        Role:<select name="role" required>
                <option value="1" <?php echo isset($_POST['role']) && $_POST['role'] == 1 ? 'selected' : ''; ?>>User</option>
                <option value="5" <?php echo ($_SESSION["role"] !== 5) ? 'disabled' : ''; ?><?php echo isset($_POST['role']) && $_POST['role'] == 5 ? 'selected' : ''; ?>>Admin</option>
            </select>
    ...
    if(isset($_POST['submit'])){
        ...
        $userClass->edit($username,$name,$surname,$email,$password,"",$role);
    }
    ?>
</div>
